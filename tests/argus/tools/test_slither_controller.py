...
class TestSlitherController:
    """Tests for SlitherController class."""

    @pytest.fixture
    def controller(self) -> SlitherController:
        """Create a SlitherController instance for testing."""
    ...
    async def test_image_pull_failure(
        ...
        mock_config.get.side_effect = lambda key, default: {
            ...
            "tools.slither.docker.network_mode": "none",
            "tools.slither.docker.remove_containers": True,
            "tools.slither.timeout": 300,
            "tools.slither.format": "json",
    ...
    async def test_successful_execution_with_json_output(
        ...
        assert '{"success": true, "results": []}' in result["output"]
        ...
        assert call_args[0] == "trailofbits/eth-security-toolbox:latest"  # image
        assert isinstance(call_args[1], list)  # command as list
        assert "slither" in call_args[1]
    ...
    async def test_default_target_to_project_root(
        ...
        result = await controller.execute(command="slither", args=[])
