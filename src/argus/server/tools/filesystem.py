...
async def list_directory(
    ...
    try:
        ...
        if directory_path is None:
            path = Path(conf.get("workdir", Path.cwd().as_posix()))
        else:
...
async def find_files_by_extension(
    ...
    try:
        ...
        if directory is None:
            search_dir = Path(conf.get("workdir", Path.cwd().as_posix()))
        else:
...
async def read_file_info(file_path: str) -> Dict[str, Any]:
    """
    ...
    Examples:
        ...
        get_file_info(file_path="contracts/Token.sol")

        # Get info about a directory
        get_file_info(file_path="test")
    """
    try:
        # Resolve path
        path = Path(file_path)
